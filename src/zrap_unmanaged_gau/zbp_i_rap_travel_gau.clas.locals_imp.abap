...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD create.
    ...
    DATA entity_in  TYPE /dmo/travel.
    DATA entity_out TYPE /dmo/travel.
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).

      entity_in = CORRESPONDING #( <entity> MAPPING FROM ENTITY USING CONTROL ).

      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_CREATE'
        EXPORTING
          is_travel   = CORRESPONDING /dmo/s_travel_in( entity_in )
        IMPORTING
          es_travel   = entity_out
          et_messages = messages.

      IF messages IS INITIAL.
        APPEND VALUE #( %cid = <entity>-%cid travelid = entity_out-travel_id ) TO mapped-travel.
      ELSE.

        "fill failed return structure for the framework
        APPEND VALUE #( travelid = entity_in-travel_id ) TO failed-travel.
        "fill reported structure to be displayed on the UI
        APPEND VALUE #( travelid = entity_in-travel_id
  ...
  METHOD update.
    ...
    DATA entity_in   TYPE /dmo/travel.
    DATA entity_x  TYPE /dmo/s_travel_inx . "refers to x structure (> BAPIs)
    DATA messages TYPE /dmo/t_message.

    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).

      entity_in = CORRESPONDING #( <entity> MAPPING FROM ENTITY ).
      entity_x-travel_id = <entity>-TravelID.
      entity_x-_intx = CORRESPONDING zsrap_travel_x_gau( <entity> MAPPING FROM ENTITY ).
      ...
      IF messages IS INITIAL.
        ...
        APPEND VALUE #( travelid = entity_in-travel_id ) TO mapped-travel.
  ...
  METHOD read.
    ...
    LOOP AT keys INTO DATA(key) GROUP BY key-TravelId.
      ...
      IF messages IS INITIAL.
        ...
        INSERT CORRESPONDING #( entity_out MAPPING TO ENTITY ) INTO TABLE result.
  ...
  METHOD lock.
    ...
    TRY.
        ...
        LOOP AT keys ASSIGNING FIELD-SYMBOL(<key>).
          ...
          ENDTRY.
        ENDLOOP.
      CATCH cx_abap_lock_failure INTO DATA(lx_abap_lock).
    ENDTRY.
  ENDMETHOD.

  METHOD rba_Booking.

    DATA: parent_entity_out TYPE /dmo/travel,
          entities_out      TYPE /dmo/t_booking,
    ...
    LOOP AT keys_rba  ASSIGNING FIELD-SYMBOL(<key_rba>) GROUP  BY <key_rba>-TravelId.
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_READ'
        ...
        IMPORTING
          es_travel    = parent_entity_out
          et_booking   = entities_out
          et_messages  = message.

      IF message IS INITIAL.

        LOOP AT entities_out ASSIGNING FIELD-SYMBOL(<fs_booking>).
...
CLASS lsc_ZI_RAP_TRAVEL_U_GAU DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_RAP_TRAVEL_U_GAU IMPLEMENTATION.
